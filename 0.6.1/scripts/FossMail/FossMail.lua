-- FossMail - Release 1 - For tes3mp 0.6.1

--[[ INSTALLATION
1) Save this file as "FossMail.lua" in mp-stuff/scripts
2) Add [ FossMail = require("FossMail") ] to the top of server.lua
3) Add the following to the elseif chain for commands in "OnPlayerSendMessage" inside server.lua
[ elseif cmd[1] == "mail" then
    FossMail.OnCommand(pid) ]
4) Add the following to OnGUIAction in server.lua
  [ if FossMail.OnGUIAction(pid, idGui, data) then return end ]
5) Add the following to the end of OnServerPostInit in server.lua
  [ FossMail.OnServerPostInit() ]
6) Add the following to directly after "myMod.OnPlayerConnect(pid, playerName)" inside OnPlayerConnect in server.lua
  [ FossMail.OnPlayerConnect(pid) ]
]]
...
Methods.OnServerPostInit = function()
  local file = io.open(os.getenv("MOD_DIR") .. "/FossMail.json", "r")
  if file ~= nil then
    io.close()
    Load()
  else
...
local function loginNotify(pid)
  ...
  for key, mdata in pairs(mailData.messages) do
...
local function sendMailMessage(mdata)
  ...
  if mdata.generated ~= true then
    if not tableHelper.isEmpty(mdata.attachments) then
      ...
      local sendPlayer = myMod.GetPlayerByName(mdata.sender)
      local hasItems = true
...
showComposingItemRemoveNumberPrompt = function(pid)
  ...
  return tes3mp.InputDialog(pid, config.composingItemRemoveNumberPrompt, message)
end
...
showComposingItemAddNumberPrompt = function(pid)
  ...
  return tes3mp.InputDialog(pid, config.composingItemAddNumberPrompt, message)
end
...
showComposingRecipient = function(pid)
  ...
  return tes3mp.InputDialog(pid, config.composingRecipientGUI, message)
end
...
showComposingText = function(pid)
  ...
  return tes3mp.InputDialog(pid, config.composingTextGUI, message)
end
...
showComposingSubject = function(pid)
  ...
  return tes3mp.InputDialog(pid, config.composingSubjectGUI, message)
end
...
local function onComposingSend(pid)
  ...
  if myMod.GetPlayerByName(mdata.target) == nil then --The target player doesn't exist
...
showViewList = function(pid)
  ...
  for i, mailId in ipairs(unreadMessages) do
    table.insert(ultimateSorted, mailId)
  end
  for i, mailId in ipairs(readMessages) do
    table.insert(ultimateSorted, mailId)
  end
  ...
  for i, mailId in ipairs(ultimateSorted) do
...
Methods.OnGUIAction = function(pid, idGui, data)
  ...
  elseif idGui == config.composingItemAddGUI then -- Composing: Add Item List
    if tonumber(data) == 0 or tonumber(data) == 18446744073709551615 then --Close/Nothing Selected
      --Do nothing
      return true, showComposingGUI(pid)
    else
  ...
  elseif idGui == config.composingItemAddNumberPrompt then -- Composing: Add Item Count Prompt
    if tonumber(data) ~= nil then --Valid number entered
      return true, onComposingItemAddNumberPrompt(pid, tonumber(data))
    else
      return true, showComposingItemAddNumberPrompt(pid) --Prompt again until a valid number is entered
    end
  ...
  elseif idGui == config.composingItemRemoveNumberPrompt then -- Composing: Remove Attachment Number Prompt
    if tonumber(data) ~= nil then --Valid number entered
      return true, onComposingItemRemoveNumberPrompt(pid, tonumber(data))
    else
      return true, showComposingItemRemoveNumberPrompt(pid) --Prompt again until a valid number is entered
    end
  end
end
...
Methods.RegisterFunction = function(scriptName, funcName, func)
...
Methods.SendMessage = function(messageData)
